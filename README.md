...
├── src/main/java/com/example/redispatterns/
│   ├── config/                  # 설정 클래스
│   │   ├── CacheConfig.java     # 캐시 설정
│   │   ├── RedisConfig.java     # RedisTemplate 및 직렬화 설정
│   │   └── WebConfig.java       # Spring Web MVC 설정
│   ├── product/                 # Cache-Aside 패턴 구현
│   │   ├── Product.java         # 상품 모델
│   │   ├── ProductRepository.java # 느린 저장소 시뮬레이션
│   │   ├── ProductService.java  # @Cacheable 사용 서비스
│   │   └── ProductController.java # REST 컨트롤러
│   ├── payment/                 # Idempotency-Key 패턴 구현
│   │   ├── PaymentRequest.java  # 결제 요청 DTO
│   │   ├── PaymentResponse.java # 결제 응답 DTO
│   │   ├── PaymentService.java  # 멱등성 처리 로직 포함
│   │   └── PaymentController.java # 헤더를 통한 멱등성 키 처리
│   ├── ratelimit/               # Rate Limiting 패턴 구현
│   │   ├── RateLimitService.java # Lua 스크립트 기반 서비스
│   │   └── RateLimitInterceptor.java # MVC 인터셉터
│   ├── exception/               # 예외 처리
│   │   ├── ErrorResponse.java   # 표준 에러 응답
│   │   └── GlobalExceptionHandler.java # @RestControllerAdvice
│   └── RedisPatternApplication.java # 메인 애플리케이션 클래스
└── docker-compose.yml          # Redis 컨테이너 구성
```
